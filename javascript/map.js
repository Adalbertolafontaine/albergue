...
let barrios_geo = L.geoJson(mapa_barrio, {
  style: style,
  onEachFeature: onEachFeature,
  filter: filtro,
}).addTo(map);
...
SLprovincia.addEventListener("change", async (e) => {
  ...
  if (SLprovincia.value !== "00") {
    ...
    let p = await Llamar(
      `https://sdo.do/sibe/municipio?prov=${SLprovincia.value}`
    );
    ...
    let esquina = municipio_geo.getBounds();
    map.fitBounds(esquina);
  } else {

    let esquina = geojson.getBounds();
    map.fitBounds(esquina);
...
Slmunicipio.addEventListener("change", async (e) => {
  llenado_cuadro(SLprovincia.value, Slmunicipio.value, "00");

  map.removeLayer(geojson);
  map.removeLayer(municipio_geo);
  map.removeLayer(barrios_geo);

  if (Slmunicipio.value !== "00") {
  
    
    let p = await Llamar(
      `https://sdo.do/sibe/parajes?prov=${SLprovincia.value}&mun=${Slmunicipio.value}`
    );

    mapa_barrio.features = p["data"];

    barrios_geo = L.geoJson(mapa_barrio, {
      style: style,
      onEachFeature: onEachFeature,
      filter: filtro,
    }).addTo(map);
    ...
    //let p = L.control.layers;
    let t = barrios_geo.getBounds();
    map.fitBounds(t);
  } else {
    map.addLayer(municipio_geo);
  }
